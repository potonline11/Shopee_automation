...
async function ensureDefaultVideos() {
  ...
  const markerFile = path.join(videosDir, "real_people_videos_v3.txt");
  const forceOverwrite = !fs.existsSync(markerFile);

  if (forceOverwrite) {
    console.log("[Video Downloader] Cleaning up old cartoon video cache to make way for premium real-person templates...");
    ...
    try {
      fs.writeFileSync(markerFile, "v3_real_people_only", "utf-8");
    } catch (e) {}
  }

  const videoMappings = [
    { name: "fan.mp4", url: "https://assets.mixkit.co/videos/preview/mixkit-man-working-on-a-laptop-in-an-office-42321-large.mp4" },
    { name: "laundry.mp4", url: "https://assets.mixkit.co/videos/preview/mixkit-woman-working-on-a-laptop-in-an-office-42323-large.mp4" },
    { name: "phone.mp4", url: "https://assets.mixkit.co/videos/preview/mixkit-business-woman-in-office-holding-smartphone-40338-large.mp4" },
    { name: "beauty.mp4", url: "https://assets.mixkit.co/videos/preview/mixkit-woman-sitting-at-her-office-desk-working-40344-large.mp4" },
    { name: "kitchen.mp4", url: "https://assets.mixkit.co/videos/preview/mixkit-woman-working-on-a-laptop-in-an-office-42323-large.mp4" },
    { name: "pet.mp4", url: "https://assets.mixkit.co/videos/preview/mixkit-business-partners-working-at-office-desk-40340-large.mp4" },
    { name: "travel.mp4", url: "https://assets.mixkit.co/videos/preview/mixkit-video-of-a-man-working-in-a-sunny-office-40348-large.mp4" },
    { name: "default.mp4", url: "https://assets.mixkit.co/videos/preview/mixkit-man-working-on-a-laptop-in-an-office-42321-large.mp4" }
  ];
...
Generate the JSON response containing 'title', 'twistDescription', 'scenes' (array of 4 scenes with sceneNumber, visualDescription, stableImagePrompt, voiceover, soundEffect, subtitle, duration), 'captions', and 'tags' (array of strings).`;
    ...
    // Inject Image placeholders to scenes using curated professional human/lifestyle photos (Absolutely no animals or pigs)
    const REAL_PERSON_SCENE_IDS = [
      "photo-1519085360753-af0119f7cbe7", // Somchai-like professional man in office
      "photo-1573497019940-1c28c88b4f3e", // Ploy-like professional woman in workplace
      "photo-1542744173-8e0856011213", // Modern office workspace
      "photo-1522071820081-009f0129c71c", // Young professionals working together
      "photo-1517245386807-bb43f82c33c4", // Energetic team workshop / meeting
      "photo-1531538606174-0f90ff5dce83", // Professional woman using mobile phone
      "photo-1556742049-0cfed4f6a45d", // Product demo / presentation
      "photo-1516321318423-f06f85e504b3", // Online shopping / digital tablet use
      "photo-1551836022-d5d88e9218df", // Confident speaker presenting
      "photo-1486406146926-c627a92ad1ab"  // Modern high-rise city office skyscraper
    ];

    const scenesWithImages = (scriptData.scenes || []).map((scene: any) => {
      const photoIndex = ((scene.sceneNumber || 1) - 1) % REAL_PERSON_SCENE_IDS.length;
      const photoId = REAL_PERSON_SCENE_IDS[photoIndex];
      const seedVal = Math.floor(Math.random() * 100);
      return {
        ...scene,
        id: `scene_auto_${Date.now()}_${scene.sceneNumber}`,
        imageUrl: `https://images.unsplash.com/${photoId}?w=500&auto=format&fit=crop&q=80&sig=${seedVal}`
      };
    });
