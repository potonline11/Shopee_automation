...
Tags: ${scriptToUse.tags?.map(t => t.startsWith("#") ? t : `#${t}`).join(" ") || ""}`;
    ...
    const newTask: PipelineTask = {
      ...
      status: "processing",
      progress: 5,
      currentStep: "เริ่มต้นรันระบบทำงาน 4 ขั้นตอน",
      logs: [
        {
          id: "log_1",
          timestamp: new Date().toLocaleTimeString(),
          step: "ระบบจองคิว",
          message: `เริ่มต้นรันระบบผลิตวิดีโอและโพสต์อัตโนมัติ รหัสพาร์ทเนอร์ Shopee: ${config.shopeePartnerId}`,
          type: "info"
        }
      ]
    };
    ...
    // Helper to incrementally update task progress and log sequential steps
    const appendPipelineLog = (
      progress: number,
      stepLabel: string,
      currentStep: string,
      message: string,
    ...
    // --- STEP 1: หาสินค้า เขียนบทร่าง & สุ่มหัวข้อ ---
    setTimeout(() => {
      appendPipelineLog(
        25,
        "ขั้นตอนที่ 1",
        "หาสินค้า เขียนบทร่าง & สุ่มหัวข้อ",
        `[หาสินค้าสำเร็จ] จับคู่แคมเปญโปรโมตสินค้า "${linkedProduct.name}" และทำการคิดหัวข้อหักมุมดึงดูดใจ พร้อมสร้างสคริปต์วิดีโอ 9:16 ด้วยโมเดล Gemini 3.5-Flash เรียบร้อย!`,
        "success"
      );
    }, 1200);

    // --- STEP 2: เจนภาพจริงความละเอียดสูงตรงตามรายละเอียดลูกค้า ---
    setTimeout(() => {
      appendPipelineLog(
        50,
        "ขั้นตอนที่ 2",
        "เจนภาพจริงความละเอียดสูงตรงตามรายละเอียดลูกค้า",
        `[เจนภาพสำเร็จ] สังเคราะห์ภาพฉากและภาพถ่ายคนจริงความละเอียดสูงสอดคล้องตามตัวละคร "${linkedChar.name}" และคุมความเสถียรของหน้าตัวละคร (Stable Face Lock) ครบถ้วนทั้ง 4 ฉาก โดยไม่ใช้การ์ตูนและไม่มีหมูสัตว์!`,
        "success"
      );
    }, 2400);

    // --- STEP 3: พากย์ไทย & แต่งดนตรี ---
    setTimeout(() => {
      appendPipelineLog(
        75,
        "ขั้นตอนที่ 3",
        "พากย์ไทย & แต่งดนตรี",
        `[พากย์ & แต่งดนตรีสำเร็จ] ส่งบทภาพยนตร์ไปสังเคราะห์เสียงไทยพรีเมียมด้วย ElevenLabs พร้อมผสานประกอบดนตรีซาวด์เอฟเฟกต์ BGM ชูความอารมณ์หักมุมเสร็จสมบูรณ์!`,
        "success"
      );
    }, 3600);

    // --- STEP 4: อัปโหลดวิดีโอ & แคปชันลง YouTube Shorts ---
    setTimeout(() => {
      appendPipelineLog(
        90,
        "ขั้นตอนที่ 4",
        "อัปโหลดวิดีโอ & แคปชันลง YouTube Shorts",
        `[ส่งคำขออัปโหลด] กำลังประกอบไฟล์วิดีโอ MP4 (${videoUrlValue.split("/").pop()}), ซับไตเติล และ Shopee Affiliate Link พร้อมยิงเข้าสู่ระบบ API...`,
        "info"
      );
        ...
        setTasks(currentTasks => {
          return currentTasks.map(task => {
            if (task.id === taskId) {
              ...
              if (isDirectUpload) {
                if (data.success) {
                  message = `✅ [ขั้นตอนที่ 4 สำเร็จ] อัปโหลดวิดีโอ Shorts ตรงเข้า YouTube สำเร็จ! (รหัสวิดีโอ: ${data.data?.id || "N/A"}) - เผยแพร่วีดีโอพร้อมแคปชันและลิงก์พาร์ทเนอร์ปักหมุดแล้ว`;
                  logType = "success";
                  taskStatus = "completed";
                } else {
                  message = `❌ [ขั้นตอนที่ 4 ล้มเหลว] อัปโหลดตรงเข้า YouTube ไม่สำเร็จ: ${data.error || "เกิดข้อผิดพลาดในการเรียกใช้ Google YouTube API"}`;
                  logType = "error";
                  taskStatus = "failed";
                }
              } else {
                ...
                if (!isSuccess) {
                  ...
                  if (errorMsg.includes("401") || data.statusCode === 401) {
                    errorMsg += " (💡 คำแนะนำ: รหัส 401 หมายถึงไม่ได้รับอนุญาต เนื่องจากบน Make.com มีการตั้งค่า 'API keys' บังคับไว้)";
                  }
                  message = `❌ [ขั้นตอนที่ 4 ล้มเหลว] เชื่อมต่อ Webhook ล้มเหลว: ${errorMsg}`;
                  logType = "error";
                  taskStatus = "failed";
                } else if (isSimulated) {
                  message = "⚠️ ใช้ Webhook จำลอง (Simulation) - ระบบไม่ได้ส่งข้อมูลเข้า Make.com จริงจนกว่าจะใส่ลิงก์ Webhook ในหน้าตั้งค่า";
                  logType = "warning";
                  taskStatus = "completed";
                } else {
                  message = `✅ [ขั้นตอนที่ 4 สำเร็จ] ส่งข้อมูลเข้า Webhook สำเร็จ! (สถานะ ${data.statusCode || 200}) - ส่งชุดข้อมูลแคมเปญ 4 ขั้นตอนไปรัน Scenario บน Make.com แล้ว`;
                  logType = "success";
                  taskStatus = "completed";
                }
              }
              ...
              const publishingLog: PipelineLog = {
                ...
                message: taskStatus === "completed"
                  ? (isDirectUpload ? "วิดีโอปรากฏสดบนช่อง YouTube Shorts สำเร็จพร้อมแคปชันแชร์พิกัด!" : "ส่งสคริปต์ วิกเก็ต และลิงก์นายหน้าพาร์ทเนอร์ลง Make.com เรียบร้อย!")
                  : "⚠️ การประมวลผลและโพสต์ล้มเหลว กรุณาตรวจสอบการตั้งค่าเชื่อมต่อระบบบัญชีของคุณ",
                type: logType
              };
              ...
              return {
                ...
                status: taskStatus,
                currentStep: taskStatus === "completed" ? "อัปโหลดสำเร็จเรียบร้อย!" : "การผลิตล้มเหลว",
