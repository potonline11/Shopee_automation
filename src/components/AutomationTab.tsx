      ...
      {activeSubTab === "pipeline" && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
              ...
              {tasks.length === 0 ? (
                <div className="p-8 text-center text-slate-500 text-sm">
                  ไม่มีงานในคิวการผลิตขณะนี้ กดเริ่มต้นรันหรือตั้งค่า Autopilot ได้ด้านขวา
                </div>
              ) : (
                  ...
                  {tasks.map((task) => (
                    <div 
                      ...
                      className={`p-4 transition-all cursor-pointer flex flex-col md:flex-row md:items-center justify-between gap-4 ${
                        task.id === selectedTaskId ? "bg-slate-850/40 border-l-4 border-l-orange-500" : "hover:bg-slate-800/20"
                      }`}
                    >
                      <div className="space-y-1 flex-1">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-mono text-slate-500">#{task.id.slice(-5)}</span>
                          <span className="text-xs text-slate-300 font-bold truncate max-w-[250px]">{task.scriptTitle}</span>
                        </div>
                        <div className="flex items-center gap-3">
                          <span className={`text-[10px] px-2 py-0.5 rounded font-mono ${
                            task.status === "completed" ? "bg-emerald-500/10 text-emerald-400" :
                            task.status === "processing" ? "bg-purple-500/10 text-purple-400 animate-pulse" :
                            "bg-rose-500/10 text-rose-400"
                          }`}>
                            {task.status.toUpperCase()}
                          </span>
                          <span className="text-[11px] text-slate-500">
                            {task.logs[0]?.timestamp || "N/A"}
                          </span>
                        </div>
                      </div>
            ...
            {/* Selected Task Details & Logs console */}
            {activeTask && (
              <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
                <div className="border-b border-slate-800/80 p-4 bg-slate-950/40 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Terminal className="w-5 h-5 text-orange-400" />
                    <span className="text-sm font-bold text-white font-mono">TASK CONSOLE LOGS & OUTPUT (ผลลัพธ์และล็อกการรัน)</span>
                  </div>
                  <span className="text-xs bg-slate-800/80 text-orange-400 font-mono font-bold px-2.5 py-1 rounded border border-orange-400/10">
                    ID: {activeTask.id.slice(-8)}
                  </span>
                </div>

                <div className="p-5 space-y-5">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-950 p-4 rounded-xl border border-slate-800/40">
                    <div>
                      <span className="text-[10px] text-slate-500 font-mono uppercase block">หัวข้อสคริปต์ที่ประมวลผล</span>
                      <h4 className="text-sm font-bold text-white mt-0.5">{activeTask.scriptTitle}</h4>
                    </div>
                    <div className="flex gap-2">
                      <span className={`text-[10px] font-mono font-bold px-2 py-1 rounded ${
                        activeTask.status === "completed" ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" :
                        activeTask.status === "processing" ? "bg-purple-500/10 text-purple-400 border border-purple-500/20 animate-pulse" :
                  ...
                  {/* Console Terminal Window for Logs */}
                  <div>
                    <span className="text-[10px] text-slate-500 font-mono uppercase block mb-1.5">ประวัติขั้นตอนการประมวลผล (LOG TERMINAL)</span>
                    <div className="bg-slate-950 rounded-xl p-4 border border-slate-800/80 font-mono text-xs text-slate-300 space-y-1.5 max-h-48 overflow-y-auto scrollbar-thin">
                      {(activeTask.logs || []).map((log, lIdx) => (
                        <div key={lIdx} className="flex gap-2.5 items-start leading-relaxed">
                      ...
                      {(activeTask.logs || []).length === 0 && (
                        <p className="text-slate-600 italic">ไม่มีรายการล็อกในคิวนี้</p>
                      )}
                    </div>
                  </div>
