...
export default function ScriptwriterTab({
  ...
  const handleGenerateScript = async () => {
    ...
    try {
      ...
      // Curated portfolio of professional, real-person business, office, and lifestyle images (No random animals/pigs)
      ...
      // Inject some imagery into scenes based on character reference image and premium curated placeholder
      const updatedScenes = data.scenes.map((scene: Scene) => {
        const photoIndex = (scene.sceneNumber - 1) % REAL_PERSON_SCENE_IDS.length;
        const photoId = REAL_PERSON_SCENE_IDS[photoIndex];
        const sceneSeed = Math.floor(Math.random() * 100);
        return {
          ...scene,
          id: "scene_" + Date.now() + "_" + scene.sceneNumber,
          imageUrl: `https://images.unsplash.com/${photoId}?w=500&auto=format&fit=crop&q=80&sig=${sceneSeed}`
        };
      });
