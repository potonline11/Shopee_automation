...
export default function ShopeeConverterTab({ products, shopeePartnerId, onAddProduct, onSelectProduct }: ShopeeConverterTabProps) {
  ...
  // Gemini AI Product Discovery states
  const [aiProducts, setAiProducts] = useState<ShopeeProduct[]>([]);
  const [isAiLoading, setIsAiLoading] = useState(false);

  const handleFindViralProducts = async () => {
    setIsAiLoading(true);
    try {
      const res = await fetch("/api/gemini/find-viral-products");
      const data = await res.json();
      if (data.success && data.products) {
        setAiProducts(data.products);
      }
    } catch (err) {
      console.error("Failed fetching viral products from Gemini:", err);
    } finally {
      setIsAiLoading(false);
    }
  };

  const handleAddAiProduct = (prod: ShopeeProduct) => {
    onAddProduct(prod);
    // Remove from AI products list once added to avoid duplicates
    setAiProducts(prev => prev.filter(p => p.id !== prod.id));
  };
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
            ...
            <p className="text-xs text-slate-400 mb-4">
              ให้ Google Gemini SDK ช่วยวิเคราะห์และหาสินค้าป้ายยาสุดฮิตที่มีกระแสปังใน Shopee สำหรับนำไปแต่งบทโฆษณาโดยเฉพาะ
            </p>
            
            <button
              type="button"
              onClick={handleFindViralProducts}
              disabled={isAiLoading}
              className="w-full bg-orange-500/15 hover:bg-orange-500 text-orange-400 hover:text-white border border-orange-500/30 transition-all font-medium text-xs py-2.5 px-4 rounded-xl flex items-center justify-center gap-2"
            >
              {isAiLoading ? (
                <>
                  <span className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" />
                  กำลังค้นหาสินค้าเทรนด์ล่าสุด...
                </>
              ) : (
                <>
                  <Sparkles className="w-3.5 h-3.5" />
                  ค้นหาสินค้าป้ายยาอัตโนมัติ (Gemini)
                </>
              )}
            </button>

            {aiProducts.length > 0 && (
              <div className="mt-4 space-y-3 pt-4 border-t border-slate-800/80">
                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">พบสินค้าป้ายยาแนะนำโดย AI ({aiProducts.length}):</p>
                {aiProducts.map((p) => (
                  <div key={p.id} className="flex items-center gap-3 p-3 bg-slate-950 rounded-xl border border-orange-500/15 justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="max-w-[170px]">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-[11px] font-bold text-emerald-400">{p.price}</span>
                          <span className="text-[10px] text-slate-500 font-mono truncate">{p.category}</span>
                        </div>
                      </div>
                    </div>
                    <button
                      onClick={() => handleAddAiProduct(p)}
                      className="p-1.5 bg-orange-500/20 text-orange-400 hover:bg-orange-500 hover:text-white rounded-lg transition-all"
                      title="เพิ่มสินค้าชิ้นนี้เข้าคลัง"
                    >
